...
// CreateName
Odoo.prototype.createName = function (model, params, context, callback) {
...
// Resequence
Odoo.prototype.resequence = function (model, ids, context, callback) {
    this._request('/web/dataset/resequence', {
        context: context,
        model: model,
        ids: ids
    }, callback);
...
Odoo.prototype.action_switch_invoice_into_refund_credit_note = function(id, callback) {
    this._request('/web/dataset/call_kw', {
        kwargs: {
            context: this.context
        },
        model: "account.move",
        method: 'action_switch_invoice_into_refund_credit_note',
        args: [id]
    }, callback);
}

Odoo.prototype.action_post = function(model, id, callback) {
    this._request('/web/dataset/call_kw', {
        kwargs: {
            context: this.context
        },
...
Odoo.prototype.search_read = function (model, { domain = [], fields = [], limit = 0, offset = 0, sort = '' }, callback) {
    if (!fields.length) {
        return console.error("The search_read method doesn't support an empty fields array.");
    }
    ...
    this._request(`/web/dataset/call_kw/${model}/search_read`, data, callback);
};
